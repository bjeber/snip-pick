...
import { createKeySetReader, type JwksSource } from './jwks';
...
/** The slice of the server configuration a resource server needs. */
...
/**
 * Builds the resource-server guard. Verifies the access token's signature, issuer and audience,
 * so a token minted by this server for a *different* resource cannot be replayed here (RFC 8707).
 *
 ...
 */
export function createAccessTokenGuard(options: {
  config: VerifyConfig;
  jwks: JwksSource;
}): MiddlewareHandler<AuthedEnv> {
  const { config } = options;
  const reader = createKeySetReader(options.jwks);

  return createMiddleware<AuthedEnv>(async (c, next) => {
    const token = bearer(c.req.header('authorization'));
    if (!token) {
      return challenge(c, config, 'invalid_request', 'A bearer access token is required.');
    }

    try {
      const header = decodeProtectedHeader(token);
      const keys = await reader.keySet(header.kid);
      ...
      return challenge(c, config, 'invalid_token', (error as Error).message);
    }
